...
class PaymentServiceHelper {
    ...
    def extractRequestFields(WireMockServer paymentServiceWireMockServer) {
        def psRequests = paymentServiceWireMockServer.findRequestsMatching(postRequestedFor(urlEqualTo('/commerce/1.x/transactionProcessor')).build()).requests
        assert psRequests.size() == 1
        paymentServiceWireMockServer.resetRequests()
        def nvpRequestText = new XmlSlurper().parseText(psRequests.bodyAsString).depthFirst().find { node ->
