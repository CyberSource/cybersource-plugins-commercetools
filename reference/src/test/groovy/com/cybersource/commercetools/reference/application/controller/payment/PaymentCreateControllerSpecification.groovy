...
import com.cybersource.commercetools.api.extension.model.Resource
...
class PaymentCreateControllerSpecification extends Specification {

    PaymentCreateController testObj
    ...
    PaymentCreateExtensionService paymentCreateExtensionService = Mock()
    ...
    def setup() {
        testObj = new PaymentCreateController(
    ...
    def 'should return validation errors if validation fails'() {
        ...
        then:
        1 * payloadLoggerMock.log('Received payment create', payloadMock)
        1 * paymentValidatorMock.validate { it.basePayment == paymentMock } >> validationResponse
        0 * paymentCreateExtensionService.process(_)
    ...
    def 'should continue processing if validation passes'() {
        ...
        1 * payloadLoggerMock.log('Received payment create', payloadMock)
        1 * paymentValidatorMock.validate { it.basePayment == paymentMock }  >> validationResponse
        1 * paymentCreateExtensionService.process { it.basePayment == paymentMock } >> successExtensionOutputMock
    ...
    def 'should return errors and a BAD REQUEST response when the extension service returns errors'() {
        ...
        1 * payloadLoggerMock.log('Received payment create', payloadMock)
        1 * paymentValidatorMock.validate { it.basePayment == paymentMock }  >> validationResponse
        1 * paymentCreateExtensionService.process { it.basePayment == paymentMock } >> errorExtensionOutputMock
    ...
    def 'Should handle empty response from extension service'() {
        ...
        1 * payloadLoggerMock.log('Received payment create', payloadMock)
        1 * paymentValidatorMock.validate { it.basePayment == paymentMock }  >> validationResponse
        1 * paymentCreateExtensionService.process { it.basePayment == paymentMock } >> emptyExtensionOutputMock
