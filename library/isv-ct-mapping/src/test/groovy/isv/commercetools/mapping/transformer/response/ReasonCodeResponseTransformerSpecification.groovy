...
class ReasonCodeResponseTransformerSpecification extends Specification {
    ...
    @Unroll
    def 'Should return a ChangeTransactionState transaction with state #state when payment service returns reason code #reasonCode'() {
        given:
    ...
    @Unroll
    def 'Should return an extra addInterfaceInteraction action when payment service returns a failure reason code (#reasonCode)'() {
        given:
        def transactionMock = Mock(Transaction)
        transactionMock.id >> 'someTransactionId'
