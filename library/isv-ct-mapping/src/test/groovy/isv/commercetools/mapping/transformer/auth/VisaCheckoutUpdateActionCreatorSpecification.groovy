...
import isv.commercetools.mapping.transformer.response.PaymentServiceResponseAddressMapper
...
class VisaCheckoutUpdateActionCreatorSpecification extends Specification {
    ...
    PaymentServiceResponseAddressMapper addressMapper

    def 'setup'() {
        addressMapper = Mock(PaymentServiceResponseAddressMapper)
    ...
    def 'Should create update actions for a payment'() {
        ...
        setCustomFieldActions.find { it.name == 'isv_maskedPan' && it.value.textValue() == '123456...789' }
        setCustomFieldActions.find { it.name == 'isv_cardType' && it.value.textValue() == 'VISA' }
        setCustomFieldActions.find { it.name == 'isv_cardExpiryMonth' && it.value.textValue() == '02' }
        setCustomFieldActions.find { it.name == 'isv_cardExpiryYear' && it.value.textValue() == '21' }
    }
