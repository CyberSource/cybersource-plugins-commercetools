...
/**
 * Handles the mapping from a payment service transaction to a Commercetools AddTransaction action. If the payment service transaction cannot be mapped
...
public class AddTransactionMapper {
  ...
  private TransactionState mapState(TssV2TransactionsGet200ResponseApplicationInformationApplications application) {
    return StateTransformer.mapISVReasonCodeToCTTransactionState(application.getReasonCode());
  }
}
