...
| PAYMENT_GATEWAY_PAYPAL_RETURN_URL             | The URL to which PayPal will redirect the customer after a successful payment      | Required if payment.paymentMethodInfo.method is payPal                                     |
| PAYMENT_GATEWAY_PAYPAL_CANCEL_URL             | The URL to which PayPal will redirect the customer if they cancel the payment process      | Required if payment.paymentMethodInfo.method is payPal                                     |
| PAYMENT_GATEWAY_PAYPAL_FAILURE_URL             | The URL to which PayPal will redirect the customer if the payment fails.     | Required if payment.paymentMethodInfo.method is payPal                                     |
| PAYMENT_GATEWAY_SERVERLESS_DEPLOYMENT | aws, azure or gcp | Provide `aws` ,`azure` or `gcp` to support serverless deployment. If the value is `aws`, it will consider AWS deployment. If the value is `azure`, it will consider Azure deployment.If the value is `gcp` it will consider GCP deployment. Case sensitive.                                |
| PAYMENT_GATEWAY_GCP_FUNCTION_NAME                  | String value            |  Provide the name of your GCP Function specified during serverless deployment.                                                          |
